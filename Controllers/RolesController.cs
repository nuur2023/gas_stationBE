...
public class RolesController(IRoleRepository repository) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(Role model) => Ok(await repository.AddAsync(model));

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, Role model) => Ok(await repository.UpdateAsync(id, model));
