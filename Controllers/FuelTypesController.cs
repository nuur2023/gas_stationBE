...
public class FuelTypesController(IFuelTypeRepository repository) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create(FuelType model) => Ok(await repository.AddAsync(model));

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, FuelType model) => Ok(await repository.UpdateAsync(id, model));
