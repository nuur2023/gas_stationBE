...
[Authorize]
public class SubMenusController(ISubMenuRepository repository) : ControllerBase
{
    [HttpGet]
    ...
    [HttpPost]
    public async Task<IActionResult> Create(SubMenu model) => Ok(await repository.AddAsync(model));

    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, SubMenu model) => Ok(await repository.UpdateAsync(id, model));
